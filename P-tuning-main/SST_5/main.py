...
from data_utils.dataset import load_file,SST5Dataset
from data_utils.vocab import init_vocab
from p_tuning.modeling import PTuneModel
...
def construct_generation_args():
    ...
    #修改：gpt2 shared->original
    # pre-parsing args
    parser.add_argument("--relation_id", type=str, default="SST-5")
    ...
    # directories
    parser.add_argument("--data_dir", type=str, default=join(abspath(dirname(__file__)), '../data/sst_data'))
    parser.add_argument("--out_dir", type=str, default=join(abspath(dirname(__file__)), '../out/sst_data'))
    # MegatronLM 11B
...
class Trainer(object):
    def __init__(self, args):
        ...
        with open("../data/sst_data/train.txt", "r") as f:
            self.train_data = f.read()

        with open("../data/sst_data/dev.txt", "r") as f:
            self.dev_data = f.read()

        with open("../data/sst_data/test.txt", "r") as f:
        ...
        self.test_set = SST5Dataset('test', self.test_data, self.tokenizer, self.args)
        self.train_set = SST5Dataset('train', self.train_data, self.tokenizer, self.args)
        self.dev_set = SST5Dataset('dev', self.dev_data, self.tokenizer, self.args)
        os.makedirs(self.get_save_path_training(), exist_ok=True)
        os.makedirs(self.get_save_path_best(), exist_ok=True)
        ...
        self.model = PTuneModel(args, self.device, self.args.template)
    ...
    def get_save_path_best(self):
        return join(self.args.out_dir, self.args.model_name, self.get_task_name(),
                    self.args.relation_id,'best')

    def get_save_path_training(self):
        return join(self.args.out_dir, self.args.model_name, self.get_task_name(),
                    self.args.relation_id,'training')
    ...
    def train(self):
        ...
        for epoch_idx in range(100):
            ...
            if epoch_idx > -1:
                ...
                ckpt_name = "epoch_{}_dev_hit1_{}_dev_loss_{}.ckpt".format(epoch_idx, round(dev_hit1 * 100, 4),round(dev_loss,4))
                dev_dict= {'dev_hit@1': dev_hit1,
                           'dev_loss': dev_loss,
                           'dev_size': len(self.dev_set)}
                path = self.get_save_path_training()
                os.makedirs(path, exist_ok=True)
                torch.save(dev_dict, join(path, ckpt_name))
                # print("# Prompt:", self.model.prompt)
                print("# {} Checkpoint {} saved.".format(self.args.relation_id, ckpt_name))
